...
<div class="card card-xxl-stretch mb-5 mb-xl-8">
  ...
  <div class="card-header border-0 pt-5">
    <h3 class="card-title align-items-start flex-column">
      ...
      <span class="text-muted mt-1 fw-bold fs-7">Over {{ count($users) }} User</span>
    </h3>
    <div class="card-toolbar d-flex justify-content-center">
      <a href="{{ route('user.export') }}" class="btn btn-light-primary my-2" data-bs-toggle="tooltip" data-bs-placement="top" data-bs-trigger="hover" title="Ekspor ke PDF">
      ...
      <a href="/user/3/create" class="btn btn-light-primary mx-3 my-2">
        <!--begin::Svg Icon | path: icons/duotone/Communication/Add-user.svg-->
        <span class="svg-icon svg-icon-3 m-0" data-bs-toggle="tooltip" data-bs-placement="top" data-bs-trigger="hover" title="Tambah User">
      ...
      <table class="table table-row-dashed table-row-gray-300 align-middle gs-0 gy-4" id="kt_datatable_example_5">
        ...
        <thead class="text-center">
          <tr class="fw-bolder text-muted">
            ...
            <th class="min-w-150px text-start align-middle px-3" rowspan="2">Nama</th>
            <th class="min-w-100px" colspan="2">Skor</th>
            <th class="min-w-300px" colspan="2">Progress</th>
            <th class="min-w-80px align-middle px-3" rowspan="2">Actions</th>
          </tr>
          <tr class="fw-bolder text-muted">
            @foreach ($mapel as $data)
            <th class="min-w-50px">{{ $data->mapel }}</th>
            @endforeach
            @foreach ($mapel as $data)
            <th class="min-w-150px">{{ $data->mapel }}</th>
            @endforeach
          </tr>
        </thead>
        <!--end::Table head-->
        ...
        <tbody class="text-center">
          ...
          <tr>
            ...
            <td>
              <div class="d-flex align-items-center">
                ...
                <div class="d-flex justify-content-start flex-column text-start">
                  <a href="{{ route('user.show', $item->id) }}" class="text-dark fw-bolder text-hover-primary fs-6">{{ $item->name }} 
            ...
            @foreach ($mapel as $data)
              ...
              @else
                @if ($data->userSkor->where('id_user', $item->id)->first())
                  <td>
                    <span class="text-dark fw-bolder d-block fs-6">{{ $data->userSkor->where('id_user', $item->id)->first()->skor }}</span>
                  </td>
                @else
                  <td>
                    <span class="text-dark fw-bolder d-block fs-6">0</span>
            ...
            <td class="text-end">
              <div class="d-flex flex-column w-100 me-2 parent-progress">
                ...
                <div class="progress h-6px w-100">
                    <div class="progress-bar bg-primary" role="progressbar" data-valuenow="{{ count($data->materiProgress->where('id_user', $item->id)) }}" data-valuemin="0" data-valuemax="{{ count($data->materiLevel) }}"></div>
                </div>
              </div>
            </td>
            @endforeach
            <td class="px-3">
              <div class="d-flex justify-content-end flex-shrink-0">
                ...
                <form action="{{ route('user.destroy', $item->id) }}" method="post">
                  <button class="btn btn-icon btn-bg-light btn-active-color-danger btn-sm" data-bs-toggle="tooltip" title="Hapus User">
                    @csrf
                    @method('DELETE')
